...
//!
//! While frozen, every monitor shows a small translucent rounded pill at its
//! bottom-center: the modes as TABS (active highlighted), each labelled with
...
fn default_bindings_render_mode_tabs_with_their_hotkeys() {
    ...
    // at 8 px per char + per-tab padding, plus pill padding and gaps.
    assert_eq!(h, 20, "8 px text + 2 * 6 px vertical padding");
...
fn custom_bindings_change_tab_text_and_pill_width() {
    ...
    assert_eq!(wider - default_w, 11 * 8, "eleven extra characters at 8 px");
}

#[test]
fn pill_is_bottom_centered_translucent_and_highlights_the_active_tab() {
    ...
    let x0 = (frame_w - pw) / 2;
    let y0 = frame_h - ph - 24;
    // The pill is painted exactly at the bottom-center band: the first
    ...
    assert_eq!(diff, (x0 + 10, y0), "first pill pixel (top edge at the radius)");
    ...
    let pad_probe = inactive.pixel(x0 + 4, y0 + ph / 2).unwrap();
    let plain_probe = plain.pixel(x0 + 4, y0 + ph / 2).unwrap();
    assert!(
        pad_probe[0] < plain_probe[0] && pad_probe[2] < plain_probe[2],
...
fn controller_paints_the_pill_centered_on_every_monitor() {
    // Two 512x64 monitors (big enough for the pill), spotlight active.
    let captured = vec![
        (
            monitor_info(Rect::new(0, 0, 512, 64)),
            buffer_with(512, 64, |x, y| [(x & 0xFF) as u8, (y & 0xFF) as u8, 40, 255]),
        ),
        (
            monitor_info(Rect::new(-512, 0, 512, 64)),
            buffer_with(512, 64, |x, y| [200, (x & 0xFF) as u8, (y & 0xFF) as u8, 255]),
        ),
    ];
    let f = FakeFreeze::new(captured, &AppSettings::default(), Point::new(256, 20));
    let legend = Legend::from_hotkeys(&AppSettings::default().hotkeys);
    let (pw, ph) = legend.size();
    for m in 0..2 {
        let frame = f.last_present(m);
        // The pill sits at THIS monitor's bottom-center (each frame is
        ...
        let mut bare = DibBuffer::new(512, 64);
        let state = RenderState {
            spotlight: Some((Point::new(256, 20), 150)),
            ..RenderState::default()
        };
        compose_frame(
            &f.captured[m].1,
        ...
        assert_ne!(
            ...
            "monitor {m}: the pill is painted at its bottom-center"
        );
    }
}
